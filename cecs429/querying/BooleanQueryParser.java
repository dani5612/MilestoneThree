...
/**
 * re Parses boolean queries according to the basequirements of the CECS 429 project.
...
public class BooleanQueryParser {
	...
	private Literal findNextLiteral(String subquery, int startIndex) {
		...
		if(subquery.charAt(startIndex) == '-' ){
			...
			++startIndex;
			Literal recursiveLiteral = findNextLiteral(subquery, startIndex);


			startIndex = recursiveLiteral.bounds.start -1;
			lengthOut =  recursiveLiteral.bounds.length + 1;

			QueryComponent anotherComponent = new NotQuery(recursiveLiteral.literalComponent);

			return new Literal(new StringBounds(startIndex, lengthOut), anotherComponent);
		...
		if(subquery.charAt(startIndex)  == '\"' ){
			...
			if (nextQuotation > 0) {
				...
				lengthOut = nextQuotation - startIndex  + 1;
				return new Literal(
