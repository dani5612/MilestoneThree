/*
 * A corpus where the documents are loaded from a directory on the 
 * local file system. Each document is derived from FileDocument.
 * and the corpus can be configured with instructions on which files to
 * load and how to construct Document objects from them.
 * 
 * 
 * 
 * This is NOT an interface, so we are implementing the
 * methods that we saw in Document Corpus
 * 
 */
...
public class DirectoryCorpus implements DocumentCorpus {
	...
	public double getByteSize(int docId){
