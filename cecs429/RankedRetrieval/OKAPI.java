...
public class OKAPI implements   Retrieval {
    ...
    public PriorityQueue<Map.Entry<Integer, Double>> getTopIdsWithScores(Index index, int corpusSize, ArrayList<String> stemmedQuery) {
        ...
        for(String term: stemmedQuery){
            ...
            double wqt = Math.max(   0.1, Math.log(   (corpusSize - postings.size() + 0.5   )  /  ( postings.size() +0.5   )         )           );
            ...
            for(Posting p: postings ){
                ...
                Double wdt = (2.2 * p.gettftd()  )/(  1.2 * (  0.25 + (    0.75 * (index.getLdForDocId(p.getDocumentId() , 1) / index.getLdForDocId(p.getDocumentId(), 4)    )    )      )  + p.gettftd()  )    ;
